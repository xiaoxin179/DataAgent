...
## 25. DataAgent SSE 的逐事件时间线

1. 浏览器发起 GET 请求，HTTP 连接保持打开。
2. Controller 创建 `unicast` sink；此时还没有业务数据。
3. Service 异步启动 Graph，Controller 立即返回 `sink.asFlux()`。
4. WebFlux 订阅返回的 Flux，触发 `doOnSubscribe`。
5. Graph 节点产生 `StreamingOutput`。
6. `handleStreamNodeOutput` 调用 `sink.tryEmitNext(...)`。
7. sink 将事件交给它暴露的 Flux。
8. Controller 的 `filter(...)` 丢弃空文本，但保留 `complete/error`。
9. WebFlux 编码 `ServerSentEvent` 并写入 HTTP 响应。
10. 浏览器逐条收到事件，连接仍保持。
11. Graph 完成后先发送带 event 名的 complete 事件，再 `tryEmitComplete()`。
12. Reactor 发出 `onComplete`，WebFlux 关闭响应。

### 三种结束方式

| 结束方式 | 触发源 | 服务端动作 |
|---|---|---|
| 正常完成 | Graph 发出 complete | 保存历史、发 SSE complete、清理 |
| 业务异常 | Graph 发出 error | 发 SSE error、完成 sink、清理 |
| 客户端取消 | 浏览器断开 | `doOnCancel -> stopStreamProcessing -> dispose` |

### 背压在这里代表什么

模型和 Graph 可能比网络发送更快。`onBackpressureBuffer()` 会暂存尚未被下游消费的事件。它避免瞬时慢消费直接丢消息，但缓冲并非无限免费：若前端长期过慢，内存仍可能增长。这里选择它是因为一次分析流通常长度有限，并且不能随意丢失计划、SQL 或报告 chunk。

### `tryEmitNext` 的返回值必须关注

`Sinks.EmitResult` 可能表示无订阅者、流已终止、并发发送冲突等。项目在发送失败时调用 `stopStreamProcessing(threadId)`，因为继续运行 Graph 已经无法把结果交付给用户。
